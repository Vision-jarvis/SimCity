...
construction; and (3) on a 32k-event live Hacker News + GDELT corpus from our
released accumulating collector, the transfer signal is consistently
recoverable across training seeds \emph{up to a sign ambiguity}: two of three
seeds converge to the same significantly-predictive narrative ranking
($\rho = 0.96$; AUC 0.590/0.660), the third learns its mirror image ---
identifying head orientation, not data volume, as the remaining obstacle to
...
popularity baseline. On the real corpus, per-seed values (0.276/0.590/0.660)
are shown rather than averaged away: the two high seeds share one narrative
ranking ($\rho = 0.96$, both significant) while the low seed learned its
mirror image --- a sign-identifiability failure of the head, not absence of
signal (\cref{sec:results}(4)).}
\label{fig:transfer}
\end{figure}
...
(68\% of target variance) breaks this symmetry; on real data it does not. The
standard remedy --- orienting each trained head on the validation split, with
no test leakage --- is the identified next experiment. Smaller or imbalanced
corpora fare strictly worse: a 93/7 HN/news snapshot yields chance for every
seed, and at 71 transfer cases no cross-seed structure is detectable.
...
up-weighting the Hawkes task ($10\times$). (ii) The real-data bridge head suffers a sign-identifiability failure
(\cref{sec:results}(4)): the learned narrative ranking replicates across seeds
($\rho = 0.96$ between converging seeds) but its orientation is arbitrary, so
raw per-seed AUCs range 0.276--0.660. Validation-split orientation is the
identified fix but is not yet implemented (it requires dumping validation
predictions); until then the headline transfer capability is established on
the controlled benchmark only. (iii) The timing margin over static
