...
confounds) --- a task on
which any static Hawkes process is random by construction; and (3) we release an
accumulating live Hacker News + GDELT collector and report honest limitations of
a first real-data snapshot.
\end{abstract}
...
\paragraph{(3) Narrative transfer detection --- the headline result.} We ask:
...
up-weighting the Hawkes task ($10\times$). (ii) On the real HN+GDELT snapshot,
popularity dominates transfer prediction (AUC 0.648) and the bridge score is
only marginal after stratification (0.570): the 93/7 platform imbalance and 53
transfer cases underpower the test. (iii) The timing margin over static MHP,
while consistent across seeds, is small on synthetic data. Confirming the
transfer result on a balanced, large-scale real corpus is the primary open
task; the released accumulating collector is designed for it.
