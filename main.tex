...
Filippov-compliant smooth Deffuant model for agent behavioural evolution. We
establish a comprehensive experimental framework for empirical validation across
multiplex internet graphs, targeting KDD/WWW/ICWSM.
\end{abstract}
...
baseline; an ablation over $M\in[10,200]$ is reported in \cref{sec:ablations}.
...
the boundary. A sensitivity analysis confirming that steady-state opinion
clusters remain invariant for $\rho > 10^2$ is provided in Appendix~B.
...
is sacrificed and empirically quantify this tradeoff in \cref{sec:ablations}.
...
We construct a 3-month temporal multiplex graph spanning January 1 --
March 31, 2024, drawn from four sources:
\begin{itemize}[noitemsep]
  \item \textbf{Reddit}: real-time ingestion of r/news and r/politics via the
        Reddit API (posts and full comment trees).
  \item \textbf{Hacker News}: full event firehose via the official Firebase API.
  \item \textbf{GDELT}: 15-minute Global Knowledge Graph (GKG) updates.
        The exogenous shock $\mu_i(t)$ is explicitly defined as the
        \emph{tone-weighted article volume} matching the narrative's NLP
        embedding published within the $t$-th 15-minute bin. Concretely,
        for each bin $t$:
        \[
          \mu_i(t)
          = \sum_{a\in\mathcal{A}_t}
            \mathrm{sim}\!\left(\hv_c(t),\;\mathrm{emb}(a)\right)
            \cdot \mathrm{tone}(a),
        \]
        where $\mathcal{A}_t$ is the set of GDELT articles in bin $t$,
        $\mathrm{emb}(a)$ is the article's NLP embedding, and
        $\mathrm{tone}(a)$ is the GDELT GoldsteinScale-normalised tone score.
\end{itemize}

\paragraph{Chronological Splitting and Leakage Prevention.}
The dataset is partitioned \emph{strictly chronologically} into a 70/10/20
train/validation/test split (no random shuffling). To prevent temporal data
leakage, TGN memory states $\bs_v(t)$ and Hawkes event history $\cH_t$ are
completely reset at the boundaries of the validation and test sets.

\subsection{Baselines}

\begin{enumerate}[noitemsep]
  \item \textbf{Static GNN + SEIR}: GraphSAGE over temporal snapshot graphs
        paired with a standard deterministic SEIR ODE (ablates the temporal
        and stochastic components).
  \item \textbf{Vanilla TGN}~\citep{rossi2020temporal}: continuous-time dynamic
        graph model without the SEIR-Z-D bridge or Virality Head.
  \item \textbf{Standard MHP}: multivariate Hawkes with static $\alpha$ matrices
        (no graph-structural intelligence).
  \item \textbf{JODIE}~\citep{kumar2019predicting}: a competitive temporal graph
        model with trajectory-based user/item embedding, representing a recent
        strong baseline for temporal interaction prediction.
\end{enumerate}

\subsection{Evaluation Metrics}

\begin{itemize}[noitemsep]
  \item \textbf{Macro-virality}: MAE and RMSE on log-engagement regression
        ($\mathcal{L}_{\mathrm{Virality}}$).
  \item \textbf{Micro-spreaders}: Precision@$K$ and NDCG measuring how
        accurately $I(v,t)$ ranks top-$K$ super-spreaders prior to cascade
        inflection.
  \item \textbf{Temporal burst timing}: NLL of cross-platform burst
        inter-arrival times ($\mathcal{L}_{\mathrm{Hawkes}}$).
\end{itemize}
...
\begin{enumerate}[noitemsep]
  \item \textbf{Centrality substitution}: we report the empirical Precision@$K$
        degradation when substituting $\mathcal{O}(V^3)$ Betweenness for
        $\mathcal{O}(1)$ Temporal Degree Centrality, assessing whether the
        computational gains justify the topological trade-off.
  \item \textbf{Deffuant smoothness} ($\rho$): sensitivity analysis confirming
        steady-state opinion clusters are invariant for $\rho > 10^2$
        (Appendix~B).
  \item \textbf{InfoNCE negative sampling} ($M$): ablation over
        $M\in\{10,20,50,100,200\}$ reporting the tradeoff between
        $\mathcal{L}_{\mathrm{TGN}}$ tightness and wall-clock training time,
        verifying stabilisation at $M=20$.
\end{enumerate}
...
[Sensitivity plots for $\rho \in \{10^0, 10^1, 10^2, 10^3\}$ demonstrating
invariance of steady-state opinion clusters for $\rho > 10^2$. To be completed
upon empirical runs.]
