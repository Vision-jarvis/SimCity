...
study a prediction task that follows directly from this: given a narrative
observed so far on a \emph{single} platform, will it later cross to another?
We approach it with a Temporal Graph Network whose memory conditions the
excitation matrix of a multivariate Hawkes process, so that each narrative
receives its own cross-platform transfer parameters. Three findings follow.
First, a negative result: the de-facto virality metric --- regressing future
engagement counts --- is intrinsically noise-dominated under near-critical
cascade dynamics; an oracle handed the true excitation state still attains
\emph{negative} skill against a per-narrative mean, so magnitude regression
cannot discriminate models and transfer should be evaluated instead. Second,
narrative-conditioned excitation predicts transfer on a controlled benchmark
(AUC $0.653 \pm 0.005$, robust to popularity and reach confounds) and on a
32k-event live Hacker News + GDELT corpus ($0.778 \pm 0.104$), beating a
per-narrative Hawkes baseline fitted independently per narrative under a
matched protocol. Third, we diagnose a sign non-identifiability of the
conditioned head --- independently trained models recover the same narrative
ranking up to a mirror image --- and resolve it with leakage-free
validation-split orientation. We release the model, an accumulating live
collector, and the evaluation suite, and report explicitly which components of
the architecture are and are not empirically load-bearing.
\end{abstract}
...
\begin{itemize}[leftmargin=*,noitemsep,topsep=2pt]
  ...
  \item \textbf{Narrative transfer detection
    (\cref{sec:results}(3),(4)).} Narrative-conditioned excitation ranks
    which single-platform narratives will later cross, on a controlled
    benchmark (AUC $0.653 \pm 0.005$) and on a live Hacker News + GDELT corpus
    ($0.778 \pm 0.104$). Crucially it beats a \emph{per-narrative} Hawkes
    baseline --- independent MLE fits, no graph conditioning --- under a
    protocol giving both sides one validation-selected degree of freedom,
    isolating the contribution of amortisation across narratives.
  \item \textbf{A diagnosed and repaired identifiability failure
    (\cref{sec:results}(4)).} The likelihood does not pin the sign of the
    conditioned head: independently trained models recover the same ranking up
    to a mirror image ($\rho = 0.96$). We show this directly
    (\cref{fig:sign}) and resolve it by fixing orientation on a validation
    split, with no test leakage.
  \item \textbf{Released artifacts and an honest scope statement.} We release
    ...
    accumulating live collector, and the full evaluation suite. We also state
...
popularity baseline. Real-corpus values are per-seed and
\emph{validation-oriented}: the likelihood does not identify the head's sign
(seeds agree up to mirror image, $\rho = 0.96$), so each head's orientation is
fixed on the validation split only --- no test leakage --- yielding
0.882/0.673/0.779, every seed individually significant
(\cref{sec:results}(4)). The popularity baseline is anti-predictive on the
real corpus.}
\label{fig:transfer}
\end{figure*}
...
\centering
\small
\caption{\textbf{Narrative transfer detection}: ranking which single-platform
narratives will later cross platforms, scored causally from pre-switch events.
The decisive comparator is the \emph{per-narrative} Hawkes baseline (row 3):
independent MLE fits with no graph conditioning, given exactly one
validation-selected degree of freedom, matching SimCity's.}
\label{tab:transfer}
\begin{tabular}{lcc}
\toprule
\textbf{Scorer} & \textbf{Synthetic AUC} & \textbf{Real AUC} \\
\midrule
SimCity bridge score & $\mathbf{0.653 \pm 0.005}$ & $\mathbf{0.778 \pm 0.104}$ \\
\quad within event-count strata & $0.656$--$0.662$ & $0.685$--$0.894$ \\
\quad within latent-reach strata & $0.656$--$0.661$ & --- \\
Per-narrative Hawkes (val-selected) & --- & $0.631$ \\
\quad \emph{oracle} statistic (test-selected) & --- & \emph{0.728} \\
Popularity baseline (event count) & $0.551$ & $0.424$ \\
Static MHP (single global $\alpha$) & \multicolumn{2}{c}{$0.500$ (by construction)} \\
\bottomrule
\end{tabular}
\\[2pt]
\footnotesize Synthetic: Mann--Whitney $p < 1.1\times10^{-4}$ every seed,
bootstrap CI $[0.58, 0.73]$, $n=209$ (123 transfer). Real: $n=259$ (166
transfer); SimCity beats the val-selected per-narrative baseline by
$+0.250$/$+0.042$/$+0.147$ across seeds (paired bootstrap CI excludes zero on
two of three). The \emph{oracle} row --- best per-narrative statistic chosen on
the test labels, an upper bound unavailable in practice --- lies inside
SimCity's seed spread, so we do not claim a large margin.
\end{table*}
...
(statistic and sign), matching SimCity's, it reaches AUC 0.631 on the real
corpus versus SimCity's $0.778 \pm 0.104$, with paired-bootstrap intervals
excluding zero on two of three seeds (\cref{tab:transfer}). The gap is the
value of \emph{amortisation}: the neural head learns a shared map from
narrative state to excitation parameters across all narratives, where
independent fitting has only each narrative's handful of events.
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.85\linewidth]{figures/sign_identifiability.pdf}
\caption{\textbf{Sign non-identifiability of the bridge head, shown directly}
(validation-oriented replicate run, real corpus; per-narrative bridge-score
percentile ranks; green = narrative later transferred). \emph{Left:} two
independently trained seeds recover essentially the \emph{same} narrative
ranking ($\rho = +0.84$, along the identity). \emph{Right:} the third seed
learns the \emph{mirror image} ($\rho = -0.59$, along the anti-diagonal); its
horizontal band reflects near-tied scores that this solution assigns to most
narratives. Which seeds converge to which orientation varies across runs ---
exactly what a sign-symmetric likelihood predicts --- and validation-split
orientation resolves it (3/3 seeds).}
\label{fig:sign}
\end{figure*}

\paragraph{(4) Transfer detection on real data --- signal replicated up to a
sign ambiguity (\cref{fig:sign}).} On the accumulated live HN+GDELT corpus (32{,}472 events,
40/60 platform split; 259 test narratives, 166 transfer), per-seed bridge AUCs
are 0.276 / 0.590 / 0.660 (the latter two individually significant,
$p = 8\times10^{-3}$ and $10^{-5}$). Superficially unstable --- but cross-seed
rank correlations reveal the mechanism: seeds 2 and 3 learn essentially the
\emph{same} per-narrative ranking (Spearman $\rho = +0.957$), while seed 1
learns its mirror image ($\rho = -0.49/-0.64$ against the others). The
transfer signal is thus \emph{consistently recoverable} from real data; what
the Hawkes likelihood fails to pin down is the \emph{orientation} of the
narrative-conditioned bridge head, whose sign-symmetric parameterisations fit
the likelihood equally well. On the synthetic benchmark the excitation signal
(68\% of target variance) breaks this symmetry; on real data it does not.
Applying the standard remedy --- orienting each trained head on the
\emph{validation} split, with no test leakage --- resolves it completely: the
validation sign predicts the correct test-side orientation for 3/3 seeds,
yielding oriented test AUCs of \textbf{0.882 / 0.673 / 0.779}
($0.778 \pm 0.104$; every seed individually significant, Mann--Whitney
$p \le 1.2\times10^{-8}$; count-stratified 0.685--0.894; the popularity
baseline is anti-predictive at 0.424 on this corpus). The real-data transfer
signal is, if anything, stronger than the synthetic benchmark's. Smaller or
imbalanced corpora fare strictly worse: a 93/7 HN/news snapshot yields chance
for every seed, and at 71 transfer cases no cross-seed structure is
detectable. Raw (unoriented) per-seed values are reported in the released
...
rather than letting the architecture diagram imply otherwise. (i) The
multi-task objective dilutes the Hawkes head: with uniform Kendall weighting
the neural model \emph{underperforms} the static MHP on timing (AUC 0.576);
recovering the advantage requires up-weighting the Hawkes task ($10\times$).
This is a tuning-sensitivity we did not resolve, only characterise. (ii) The bridge head's orientation is not identified by the likelihood on real
data (\cref{sec:results}(4)); we resolve it with validation-split orientation,
but a parameterisation that is sign-identified by construction (e.g.\ an
orientation prior or anchored excitation) would be cleaner and remains future
work. Oriented real-data AUC also varies more across seeds ($\pm 0.104$) than
the synthetic benchmark ($\pm 0.005$). (iii) The timing margin over static
...
remains future work pending API access. (v) \textbf{Narrative-cluster validity
on the real corpus is a live threat.} Sampling cross-source pairs from our
clusters (one HN title, one news title from the same narrative) showed they
were frequently unrelated: only 2--3\% of sampled pairs exceeded a $0.5$
title-embedding cosine. The cause was a defect in our own clustering code ---
once a cluster-count cap was reached, remaining items were assigned to their
nearest centroid \emph{regardless of similarity}. We have corrected the
clusterer (no forced assignment; time-windowed clusters, since a narrative is
a time-bounded event) and are re-deriving the corpus, but the real-corpus
transfer figures should be read as \emph{provisional} until recomputed over
validated narratives. The synthetic benchmarks are unaffected, as narrative
identity there is ground truth by construction. We report this because the
cheapest diagnostic in our revision --- manually inspecting a sample of our own
data --- found the most serious defect in the paper.
